(function () {
  ...
  const targetURL = thisScript.getAttribute("data-url");

  if (!targetURL) {
    console.warn("[Welo Badge] ⚠ Nessun data-url trovato. Verrà usato welobadge.com come fallback.");
  } else {
    console.log("[Welo Badge] ✅ URL caricato correttamente:", targetURL);
  }

  const finalURL = targetURL || "https://welobadge.com";
    ...
    modal.innerHTML = `
      <div style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:1000;display:flex;align-items:center;justify-content:center;">
        <div style="position:relative;width:90%;max-width:800px;height:80%;background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,0.3);">
          ...
          <iframe src="` + finalURL + `" style="width:100%;height:100%;border:none;" loading="lazy"></iframe>
        </div>
      </div>
    `;
