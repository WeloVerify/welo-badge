(function () {
  ...
  // ---------- Network warmup (preconnect/prefetch) ----------
  function addLink(rel, href, crossOrigin) {
    if (!href) return;
    const safeId = String(href).replace(/[^a-z0-9]+/gi, "-").slice(0, 80);
    const id = `welo-${rel}-${safeId}`;
  ...
  function initBadge() {
    ...
    const companyName =
      sanitizeCompanyName(forcedCompany || deriveCompanyName(targetURL)) || "Company";

    // ---------- Copy (EN) ----------
    const TITLE_TEXT = "Welo Badge Certification";
    const ROTATION = [
      { text: `See why ${companyName} is safe`, duration: 4200 },
      { text: "Read verified reviews", duration: 4200 },
      { text: "Open the official Trust Center", duration: 4200 },
    ];
    ...
    wrap.innerHTML = `
      ...
      <button class="welo-badge-dismiss" type="button" aria-label="Close badge">
      ...
      <div class="welo-modal" role="dialog" aria-modal="true" aria-label="Welo Badge">
        <div class="welo-modal-header">
          ...
          <div class="welo-header-buttons">
            <button class="welo-open-btn" id="welo-open-btn" type="button" title="Open Welo Page">
            ...
            <button class="welo-close-btn" id="welo-close-btn" type="button" title="Close">
              ...
              <span class="welo-btn-text">Close</span>
            </button>
          </div>
        </div>
        ...
        <div class="welo-iframe-container">
          ...
          <iframe
            ...
            data-loaded="0"
            title="Welo Page"
            style="width:100%; height:100%; border:none; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; display:block;"
            loading="lazy"></iframe>
        </div>
      </div>
    `;
