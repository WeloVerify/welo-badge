(function () {
  ...
  const ALIGN = String(thisScript.getAttribute("data-align") || "left").toLowerCase();
  ...
  function setViewportUnit() {
    ...
    document.documentElement.style.setProperty("--welo-vh", `${Math.max(1, h * 0.01)}px`);
  }

  // ─── Network warmup ───────────────────────────────────────────────────────
  function addLink(rel, href, crossOrigin) {
    if (!href) return;
    const id = `welo-${rel}-${String(href).replace(/[^a-z0-9]+/gi, "-").slice(0, 80)}`;
  ...
  function deriveCompanyName(url) {
    try {
      ...
      slug = slug
        ...
        .trim();
      if (!slug) {
        const base = (u.hostname || "").replace(/^www\./i, "").split(".")[0] || "Azienda";
  ...
  function initBadge() {
    ...
    modal.innerHTML = `
      <div class="welo-modal" role="dialog" aria-modal="true" aria-label="Welo Badge">
        ...
        <div class="welo-iframe-container">
          ...
          <iframe id="welo-iframe" data-loaded="0" title="Welo Page"
    ...
    if (!document.getElementById("welo-badge-style")) {
      ...
      style.innerHTML = `
        ...
        .welo-badge-wrap,.welo-badge-wrap *{box-sizing:border-box;}
        .welo-badge-wrap{
          position:fixed;
        ...
        .welo-badge-wrap.welo-align-right{left:auto;right:var(--welo-side,20px);--welo-tx:0px;}
        .welo-badge-wrap.welo-align-center{left:50%;right:auto;--welo-tx:-50%;}
        .welo-badge-wrap.is-visible{opacity:1;transform:translate3d(var(--welo-tx,0px),0,0);pointer-events:auto;}
        .welo-pill{
          height:56px;width:56px;border-radius:999px;overflow:hidden;
          display:flex;align-items:center;
          background:#fff;border:1px solid var(--welo-stroke);box-shadow:var(--welo-shadow);
          cursor:pointer;-webkit-tap-highlight-color:transparent;
          transition:width 560ms cubic-bezier(0.22,1,0.36,1),box-shadow 200ms ease;
        }
        .welo-badge-wrap.is-expanded .welo-pill{width:var(--welo-pill-width);}
        .welo-pill:hover{box-shadow:var(--welo-shadow-hover);}
        .welo-logo-container{width:56px;height:56px;flex:0 0 56px;display:flex;align-items:center;justify-content:center;}
        .welo-logo{width:28px;height:28px;object-fit:contain;transition:transform 220ms ease;}
        ...
        .welo-badge-wrap.is-expanded .welo-text{opacity:1;transform:translateX(0);}
        .welo-title{display:block;font-size:16px;font-weight:600;color:#141414;line-height:1.22;white-space:nowrap;overflow:hidden;text-overflow:clip;}
        .welo-subtitle{
          display:block;font-size:12px;font-weight:400;color:#8A8A8A;line-height:1.22;
        ...
        .welo-badge-dismiss{
          ...
          appearance:none;-webkit-appearance:none;padding:0;margin:0;touch-action:manipulation;
        }
        .welo-badge-dismiss svg{width:14px;height:14px;display:block;stroke:#111;stroke-width:2.8;stroke-linecap:round;fill:none;}
        .welo-badge-wrap.is-expanded .welo-badge-dismiss{opacity:1;pointer-events:auto;}
        .welo-overlay{
        ...
        .welo-modal{
          width:95%;max-width:1200px;height:90%;max-height:800px;
        ...
        .welo-modal.fullscreen{
          width:100vw;height:calc(var(--welo-vh,1vh)*100);
          max-width:100vw;max-height:calc(var(--welo-vh,1vh)*100);
          border-radius:0;transform:none;position:fixed;inset:0;margin:0;
        }
        .welo-modal-header{
          display:flex;align-items:center;justify-content:space-between;
          padding:15px 20px;border-bottom:1px solid #eeeeee;
          background:var(--welo-header-bg);flex-shrink:0;height:56px;
          position:relative;z-index:15;
        }
        .welo-header-title{display:flex;align-items:center;font-size:14px;font-weight:600;color:#1a1a1a;white-space:nowrap;}
        .welo-header-buttons{display:flex;align-items:center;gap:8px;}
        .welo-open-btn,.welo-fullscreen-btn,.welo-close-btn{
          ...
          font-size:13px;font-weight:500;gap:6px;min-height:30px;line-height:1;
          -webkit-appearance:none;appearance:none;
        }
        .welo-open-btn{color:#0285FF;background:rgba(2,133,255,0.08);}
        .welo-open-btn:hover{background:rgba(2,133,255,0.12);transform:translateY(-1px);}
        .welo-fullscreen-btn:hover,.welo-close-btn:hover{background:#ededed;color:#1a1a1a;}
        .welo-open-btn svg,.welo-fullscreen-btn svg,.welo-close-btn svg{stroke-width:2;flex:0 0 auto;}
        .welo-btn-text{display:inline-block;white-space:nowrap;}
        .welo-iframe-container{flex:1;min-height:0;background:#fff;position:relative;z-index:1;}
        .welo-skeleton{
          position:absolute;inset:0;background:#fff;z-index:10;
          display:flex;align-items:flex-start;justify-content:center;
          padding:22px 16px;opacity:1;transition:opacity .22s ease;
        }
        .welo-skeleton.is-hidden{opacity:0;pointer-events:none;}
        .welo-skel-wrap{
          width:min(1100px,100%);display:flex;flex-direction:column;gap:16px;
          font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;
        ...
        .welo-skel-line{height:14px;border-radius:999px;width:55%;}
        .welo-skel-line-lg{width:72%;height:16px;}
        .welo-skel-line-xl{width:92%;height:18px;}
        .welo-skel-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;}
        .welo-skel-card{height:110px;border-radius:18px;}
        .welo-skel-avatar,.welo-skel-line,.welo-skel-card{background:#eee;position:relative;overflow:hidden;}
        .welo-skel-avatar::after,.welo-skel-line::after,.welo-skel-card::after{
          content:"";position:absolute;inset:0;transform:translateX(-60%);
          background:linear-gradient(90deg,transparent,rgba(255,255,255,.55),transparent);
          animation:weloShimmer 1.1s infinite;
        }
        @keyframes weloShimmer{0%{transform:translateX(-60%);}100%{transform:translateX(60%);}}
        @media(max-width:767px){
          .welo-skel-grid{grid-template-columns:repeat(2,1fr);}
          .welo-skel-card{height:96px;}
        }
        @media(max-width:768px){
          .welo-badge-wrap{bottom:var(--welo-bottom-m,18px);}
          .welo-badge-wrap.welo-align-left{left:var(--welo-side-m,18px);right:auto;}
          .welo-badge-wrap.welo-align-right{right:var(--welo-side-m,18px);left:auto;}
          .welo-badge-wrap.welo-align-center{left:50%;right:auto;}
          ...
          .welo-badge-dismiss{width:24px;height:24px;}
          .welo-badge-dismiss svg{width:13px;height:13px;}
          .welo-modal{width:95%;max-width:1200px;height:90%;max-height:800px;border-radius:16px;}
          ...
          .welo-open-btn,.welo-fullscreen-btn,.welo-close-btn{padding:6px 8px;min-height:30px;font-size:12px;}
        }
        @media(prefers-reduced-motion:reduce){
          .welo-badge-wrap,.welo-pill,.welo-text,.welo-subtitle{transition:none !important;}
          .welo-skel-avatar::after,.welo-skel-line::after,.welo-skel-card::after{animation:none !important;}
        }
      `;
    ...
    function measureTextPx(text, font, letterSpacingPx) {
      ...
      const extra = letterSpacingPx > 0 ? letterSpacingPx * Math.max(0, String(text).length - 1) : 0;
      const w = base + extra;
    ...
    function computePillWidth() {
      ...
      const padRight = textWrap ? parseFloat(getComputedStyle(textWrap).paddingRight || "0") : (isMobile ? 12 : 15);
      ...
      let w = logoW + Math.max(titleW, subMax) + padRight + 10;
      w = Math.max(220, Math.min(Math.min(520, window.innerWidth - 44), Math.ceil(w)));
    ...
    const onResize = () => {
      ...
      resizeT = setTimeout(() => { setViewportUnit(); computePillWidth(); }, 120);
    };
    ...
    if (document.fonts && document.fonts.ready) {
      document.fonts.ready.then(() => { computePillWidth(); setTimeout(computePillWidth, 150); });
    } else {
    ...
    function animateSubtitleTo(text) {
      ...
      setTimeout(() => {
        ...
        requestAnimationFrame(() => subEl.classList.remove("welo-subtitle-prein"));
      }, 320);
    }
    ...
    // ── Skeleton ──
    function showSkeleton() { if (skel) skel.classList.remove("is-hidden"); }
    function hideSkeleton() {
      if (!skel) return;
      skel.classList.add("is-hidden");
      setTimeout(() => { try { if (skel && skel.parentNode) skel.parentNode.removeChild(skel); } catch (e) {} }, 280);
    }

    function ensureIframeLoaded() {
      if (!iframe) return;
      showSkeleton();
      if (iframe.getAttribute("data-loaded") === "1") { hideSkeleton(); return; }
    ...
    function openWeloModal() {
      ...
      requestAnimationFrame(() => {
        ...
        lockBody();
        setTimeout(() => ensureIframeLoaded(), 0);
    ...
    if (dismissBtn) {
      ...
      dismissBtn.addEventListener("click", (e) => { stopAll(e); collapseToCircle(); }, true);
    }
    ...
    if (pill) {
      ...
      pill.addEventListener("keydown", (e) => {
        if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openWeloModal(); }
      });
    }
    ...
    modal.addEventListener("click", (e) => { if (e.target === modal) closeWeloModal(); });
